...
def load_data():
    ...
    data_path = os.path.join(base_dir +'/ia_dev/data/cleaned_customer_loyalty_data.csv')
...
def get_cluster_summary(df):
    ...
    # Étape 3 : calculer le "meilleur groupe" selon ta formule
    ...
    # Étape 3 : Arrondir les colonnes "Depense" et "Score"
    # Les nombres flottants nécessitant 2 chiffres après la virgule
    summary["Depense_moyenne"] = summary["Depense_moyenne"].round(2)
    summary["Depense_min"] = summary["Depense_min"].round(2)
    summary["Depense_max"] = summary["Depense_max"].round(2)
    summary["Score_segment"] = summary["Score_segment"].round(2)


    # Étape 2 : convertir Age_moyen en entier (arrondi)
    summary["Age_moyen"] = summary["Age_moyen"].round().astype(int) 
...
if submit_button:
    if Total_Spent <= 0 or Quantity <= 0 or Avg_Price <= 0:
        ...
        if model is not None:
            ...
            st.dataframe(result_df.style.highlight_max(axis=1, subset=['probabilité_fidel'], color='blue')
            .highlight_min(axis=1, subset=['probabilité_non_fidel'], color='yellow'))
...
#st.write(f'La précision moyenne en validation croisée est : {scores.mean():.2f}')
...
if not df.empty:
    ...
    if st.sidebar.checkbox("Afficher la précision du modele"):
        st.subheader("Précision du modèle")
        st.write(f'La précision du modèle RandomForestClassifier est : {accuracy:.2f}')            

        # Affichage dans Streamlit
    if st.sidebar.checkbox("Afficher la précision du mode"):
